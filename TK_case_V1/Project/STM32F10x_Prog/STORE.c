...
u8 cmmmg;  //²âÊÔÊ¹ÓÃ
/****************************************************
* ÀûÓÃëpÖ÷´æƒ¦Ä£‰K·ÀÖ¹”µ“þGÊ§
* Ö÷ƒÈ´æ1Ó‹”µ…^Óò		           0x79-0x7C
* Ö÷ƒÈ´æ2Ó‹”µ…^Óò		           0x79-0x7C
* ×ÓƒÈ´æÓ‹”µ…^Óò 		           0x79-0x7C

* ×ÓƒÈ´æÒÑ·ÖÅäÆðÊ¼Öµ˜ËÕI           0x78    ¡¾0xCC¡¿
* Ö÷ƒÈ´æ1ÒÑ·ÖÅäÆðÊ¼Öµ˜ËÕIÎ»        0x78    ¡¾0XCC¡¿
* Ö÷ÄÚ´æ2ÒÑ·ÖÅäÆðÊ¼Öµ˜ËÕIÎ»        0x78    ¡¾OxCC¡¿

* ×ÓƒÈ´æÒÑ·ÖÅä±àºÅ˜ËÕI             0x80    ¡¾0xCC¡¿
* Ö÷ÄÚ´æ´æ´¢ÒÑ·ÖÅä×ÓÄ£¿é¸öÊý       0x81-0x84
* ×ÓÄÚ´æ´æ´¢×ÔÉí±àºÅ		       0x81-0x84

* ×ÓÄÚ´æÊÙÃü¼ì²âÇøÓò           0x70
* ×ÓÄÚ´æÀÛ¼Æ¼ÆÊýÇøÓò           0x79-0x7C
* ×ÓÄÚ´æÆðÊ¼µØÖ·ÇøÓò           0x89-0x8C
* ×ÓÄÚ´æÃÜÔ¿ÇøÓò               0xA1-0xA3
* ÃÜÔ¿ 0x43,0x45,0x50

* CRC1ÂëÐ£ÑéÇøÓò               0x85-0x88
* CRC2ÂëÐ£ÑéÇøÓò               0x95-0x98


* ×Ó´æƒ¦Ä£‰KµØÖ·£º Œ‘0xA0   ×x0xA1      
* Ö÷´æƒ¦Ä£‰K1µØÖ·£ºŒ‘0xAE   ×x0xAF
* Ö÷´æƒ¦Ä£‰K2µØÖ·£ºŒ‘0xA6   ×x0xA7 
*
*****************************************************/
...
unsigned long LIFE_TIMES=10000;   //IDÄ£¿éÊÙÃü10000´Î
/***************************************
*  ´æƒ¦Ä£‰K³õÊ¼»¯
*  
***************************************/
...
u32 SLAVE_EEPROM_COUNT_NUM(void)
{
	 ...
	 u32 coun1=0,coun2=0;
	 u8 markk;
	 markk=GET_SLAVE_EEPROM_COUNT_SIGNAL(Slave_count_Mark);  //ÅÐ”à×Ó´æƒ¦Ä£‰KÊÇ·ñÒÑ³õÊ¼»¯
	 if(markk!=0xcc)
	 { 
		 Allcolor(1,0xffffff);    Delays(120);
	 	 ShowTxt("60,20,100,Î´Ê¶±ðµ½´æ´¢Ä£¿é£¡£¡");	    Delays(100);//ÔÚÆÁÄ»ÉÏÌáÊ¾ÓÃ‘ôÕˆ´æÈëÓ‹”µ³õÊ¼Öµ£¡
		 while(1)			
		{BEEP_Dudu();}
	 }
//------------------------------------------------------------	 
		 coun2=Check_CRC_Code();    //½øÐÐCRCÐ£Ñé £¬·µ»Ø±àºÅ
	   DelayMs(2); 
		 coun1=SLAVE_Read_Data(ARR_SLAVE,Slave_count_P);   //»ñÈ¡±¾Æ¬²úÆ·¼´½«ÉÕÈëµÄIDÖµ
	   DelayMs(15);  
//	   if(coun1==0x00000000)    //Èç¹ûID_VALµÈÓÚÄ¬ÈÏÖµ0x00,0x00,0x00   ;ÔòÌø¹ý    
//		 {coun1++;}	           //¸ÃÐÍºÅÄ¬ÈÏÖµ¡  [ 00¡¢00¡¢00 ]
		 //ÌáÊ¾ÓÃ»§¼´½«ÒªÉÕÈëµÄÖµ

	  ShowTxt("32,0,150, ´Ø±àºÅ£º");	  			 DelayMs(100);
...
void SAVE_SLAVE_EEPROM_COUNT_NUM(u32 datat)
{
		...
		coun1=datat;   //»ñÈ¡¸ÃÆ¬²úÆ¬ÉÕÈëµÄIDÖµ
    coun1++;
	  Delays(5);
		SLAVE_Write_Data(ARR_SLAVE,Slave_count_P,coun1);	//¸üÐÂ¼ÆÊýÖµ + 1
	  DelayMs(15);
    ...
    coun4=SLAVE_Read_Data(ARR_SLAVE,Slave_count_P); 
	  ...
	  if(coun4==coun1)
		{
		coun2=SLAVE_Read_Data(ARR_SLAVE,Slave_BH_P);      //»ñÈ¡´Ø±àºÅ
		coun3=cfcrc(coun2,coun1);                         //¼ÆËã³ö²»¶Ï±ä»¯µÄCRC2
		SLAVE_Write_Data(ARR_SLAVE,CRC_code2,coun3);	    //¸üÐÂCRC2
		}
		else
		{
	    ...
	    DelayMs(10);			
			Allcolor(1,0xffffff);    Delays(120);
		  ShowTxt("32,0,150, Ä£¿éÒì³££¡");	  			 DelayMs(100); 
	 	  ShowTxt("60,20,100,Î´Ê¶±ðµ½´æ´¢Ä£¿é£¡£¡");	    Delays(100);//ÔÚÆÁÄ»ÉÏÌáÊ¾ÓÃ‘ôÕˆ´æÈëÓ‹”µ³õÊ¼Öµ£¡
		  while(1)			
		  {BEEP_Dudu();}
		}
...
*  ×Ó´æƒ¦Ä£‰K
*  ×Ó´æƒ¦Ä£‰KÊÙÃü²âÊÔ
*  Ä¿µÄ£º¼ì²éÄ£¿éÊÙÃü·¢ÉúÍ»·¢ÐÔÖÕÖ¹
*        ¼ì²âÄ£¿éÊÇ·ñÊÊÅä¸ÃÐÍºÅ
***************************************/
void ID_Model_Lifetimes_CHECK(void)
{
		u32 coun1=0,coun2=0;
	
	  coun1=EEPROMBYTE_ReadS(ARR_SLAVE,Lifetimes_check);         Delays(15);   //coun1  0~255·´¸´Ñ­»·
	  coun1++;Delays(5);
		EEPROMBYTE_WriteS(ARR_SLAVE,Lifetimes_check,coun1);        Delays(15);
	  
	  coun2=EEPROMBYTE_ReadS(ARR_SLAVE,Lifetimes_check);         Delays(15);
	
		if(coun2!=coun1)   //¶Ô±ÈÇ°ºóÖµ£¬Èç¹û²»Ò»ÖÂ£¬±íÊ¾IDÄ£¿éÊÙÃü³öÏÖÍ»·¢ÐÔÖÕÖ¹£¬ÐëÁ¢¿ÌÌáÐÑ£¡
    {	 	 
			  Allcolor(1,0xffffff);    Delays(120);
				ShowTxt("60,20,100,IDÄ£¿éÒì³££¡");	    Delays(100);
			  ShowTxt("60,20,200,ÇëÖØÊÔ»òÕß¸ü»»Ä£¿é£¡£¡");
        ShowTxt("60,20,300,Î´¶Ô¸Ã²úÆ·½øÐÐÉÕÈë");	
			  while(1)			
				{BEEP_Dudu();}	   
		}
//		Read_LCD_MODEL();  
...
u32 Check_CRC_Code(void)
{
   ...
   tep1=SLAVE_Read_Data(ARR_SLAVE,Slave_BH_P);	   //×ÓÐ¾Æ¬±àºÅ
   tep2=SLAVE_Read_Data(ARR_SLAVE,Slave_count_S);  //×ÓÐ¾Æ¬ÆðÊ¼¼ÆÊýÖµ
   tep3=SLAVE_Read_Data(ARR_SLAVE,Slave_count_P);  //×ÓÐ¾Æ¬ÀÛ¼Æ¼ÆÊýÖµ
	 
	 if(tep3>((tep1*LIFE_TIMES)-1))
	 {
		 Allcolor(1,0xffffff);    Delays(120);
	   ShowTxt("60,20,100,´æ´¢Ä£¿é¿Õ¼äÒÑÂú£¡");            Delays(100);
		 ShowTxt("60,20,250,Çë¸ü»»´æ´¢Ä£¿é£¡");              Delays(100);
		 ShowTxt("60,20,300,´ËÆ¬²úÆ·IDÎ´ÉÕ£¬¸üÐÂÄ£¿éºóÖØÊÔ£¡");            Delays(100);
		 ;//ÌáÊ¾ÓÃ‘ô¸ü“Q×Ó´æƒ¦Ð¾Æ¬¡£
		 while(1)			
		{BEEP_Dudu();}
	 }	 
  
   crc1=cfcrc(tep1,tep2);             //¼ÆËãÉú³ÉCRC1Âë
   crc2=cfcrc(tep1,tep3); 

	 Get_crc1=SLAVE_Read_Data(ARR_SLAVE,CRC_code1);   //»ñÈ¡CRC1Âë
	 Get_crc2=SLAVE_Read_Data(ARR_SLAVE,CRC_code2); 
	
	 if((crc1!=Get_crc1)||(crc2!=Get_crc2))
	 {
		 Allcolor(1,0xffffff);    Delays(120);
	   ShowTxt("60,20,100,Ð¾Æ¬´íÎó£¡");            Delays(100);
     //²»ÊÇ×Ô¼ÒµÄÐ¾Æ¬£¡ºß£¡½ûÖ¹Ê¹ÓÃ£¡  
		 ;//ÌáÊ¾ÓÃ‘ô¸ü“Q×Ó´æƒ¦Ð¾Æ¬¡£
		 while(1)			
		{BEEP_Dudu();}
	 }

    return tep1;   //·µ»Ø¸Ã´æ´¢Ä£¿é±àºÅ
}
/***************************************
*  ×Ó´æƒ¦Ä£‰K
*  ×xÈ¡×Ó´æƒ¦Ä£‰K®”Ç°Ó‹”µÖµ
...
void Read_LCD_MODEL(void)
{
	 ...
	 {
		 Allcolor(1,0xffffff);    Delays(120);
	   ShowTxt("60,20,100,´æ´¢Ä£¿é²»ÊÊÅä£¡");            Delays(100);
		 ;//ÌáÊ¾ÓÃ‘ô¸ü“Q×Ó´æƒ¦Ð¾Æ¬¡£
		 while(1)			
		{BEEP_Dudu();}
	 }
...
{
	SLAVE_Write_Data(ARR_SLAVE,Slave_count_P,0xffffffff);			 //¼ÆÊý
	EEPROMBYTE_WriteS(ARR_SLAVE,Slave_count_Mark,0xff);    Delays(5);	  //³õÊ¼»¯±êÖ¾Î»
	SLAVE_Write_Data(ARR_SLAVE,Slave_BH_P,0xffffffff);			 //±àºÅ
	EEPROMBYTE_WriteS(ARR_SLAVE,Slave_BH_mark ,0xff);    Delays(5);	  //±àºÅÒÑÔØÈë±êÖ¾Î»
}
